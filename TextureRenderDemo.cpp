...
private:
  GLuint program, vao1, vao2, texture_buffer, vertex_buffer, texcoord_buffer1,
      texcoord_buffer2, face_buffer;
  GLint utransform;
  double rotation_time;
  int mode;
};

Client::Client(void) : rotation_time(0), mode(0) {
  GLint value;
  ...
  if (bmp_surface == 0) cout << "failed to load 'texture.bmp'" << endl;
  unsigned char *bmp_data =
      reinterpret_cast<unsigned char *>(bmp_surface->pixels);
  int bmp_width = bmp_surface->w, bmp_height = bmp_surface->h;
  ...
  float texcoords2[8] = {1.5f, 1.5f, -1.5f, 1.5f, -1.5f, -1.5f, 1.5f, -1.5f};
...
void Client::draw(double dt) {
  const float PI = 4.0f * atan(1.0f), ROTATION_RATE = 2 * PI / 5.0f,
  ...
  if (mode % 2 == 1) rotation_time += dt;
}

void Client::keypress(SDL_Keycode kc) {
  if (kc == SDLK_SPACE) mode = (mode + 1) % 4;
}
...
void Client::mouseclick(int x, int y) { mode = (mode + 1) % 4; }
